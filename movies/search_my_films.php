<?php
  ...
  if (substr_count($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')){
	...
	// Set template
	$template_file = "my_films.tmpl.xml";
  ...
  if(mysql_num_rows($res) > 0){
    ...
    $menu_result = DBQuery("SELECT security_level FROM wmc_menus_security_levels WHERE menu_item='My Films' ORDER BY position ASC");
    ...
    if(mysql_num_rows($menu_result) > 0 && $security_level >= mysql_result($menu_result, 0, "security_level")){
      ...
      $window_check = DBQuery("SELECT id FROM wmc_users_windows WHERE user_id=" . $user_id . " AND window='MY_FILMS'");
      if(mysql_num_rows($window_check) > 0){
        DBQuery("UPDATE wmc_users_windows SET opened='1', page='search_my_films.php', parameter='search_filter=" . urlencode($search_filter) . "&char=" . $char_filter . "' WHERE user_id=" . $user_id . " AND window='MY_FILMS'");
      } else{
        DBQuery("INSERT INTO wmc_users_windows (x, y, opened, page, parameter, window, user_id) VALUES ('80', '110', '1', 'search_my_films.php', 'search_filter=" . urlencode($search_filter) . "&char=" . $char_filter . "', 'MY_FILMS', '" . $user_id . "')");
      }
      ...
      if($column_num > 0){
        for($i = 0; $i < $column_num; $i++){
          ...
          $tmpl->addGlobalVar("SHOW_LEND_TO", "none");
          $tmpl->addGlobalVar("SHOW_KEYWORD", "none");
          ...
          if(mysql_result($column_result, $i, "lend_to")){
            $show_lend_to = true;
            $tmpl->addGlobalVar("SHOW_LEND_TO", "table-cell");
          }
          if(mysql_result($column_result, $i, "keyword")){
            $show_keyword = true;
            $tmpl->addGlobalVar("SHOW_KEYWORD", "table-cell");
          }
      ...
      if($search_filter != ""){
        ...
        //Search Medium
        $medium_result = DBQuery("SELECT users_films.film_id FROM wmc_mediums AS mediums, wmc_users_films_mediums AS users_films_mediums, wmc_users_films AS users_films WHERE users_films.user_id='" . $user_id . "' AND users_films_mediums.medium_id=mediums.id AND users_films_mediums.user_film_id=users_films.id AND mediums.medium REGEXP '" . $search_filter . "' GROUP BY users_films_mediums.id ORDER BY mediums.medium ASC");
        $medium_num = mysql_num_rows($medium_result);
        for($j = 0; $j < $medium_num; $j++){
				  if($film_ids != ""){
  					$film_ids .= "|";
					}
          $film_ids .= mysql_result($medium_result, $j, "users_films.film_id");
        }

        //Search Genre
        $genre_result = DBQuery("SELECT films_genres.film_id FROM wmc_genres AS genres, wmc_films_genres AS films_genres WHERE genres.id=films_genres.genre_id AND genres.genre REGEXP '" . $search_filter . "' ORDER BY genres.genre ASC");
        $genre_num = mysql_num_rows($genre_result);
        for($j = 0; $j < $genre_num; $j++){
				  if($film_ids != ""){
  					$film_ids .= "|";
					}
          $film_ids .= mysql_result($genre_result, $j, "films_genres.film_id");
        }

        //Search Cast
        $cast_result = DBQuery("SELECT films_cast.film_id FROM wmc_cast AS casts, wmc_films_cast AS films_cast WHERE casts.id=films_cast.cast_id AND casts.name REGEXP '" . $search_filter . "' ORDER BY casts.name ASC");
        $cast_num = mysql_num_rows($cast_result);
        for($j = 0; $j < $cast_num; $j++){
				  if($film_ids != ""){
  					$film_ids .= "|";
					}
          $film_ids .= mysql_result($cast_result, $j, "films_cast.film_id");
        }
        ...
        //Search Keyword
        $keyword_result = DBQuery("SELECT film_id FROM wmc_films_keywords WHERE keyword REGEXP '" . $search_filter . "' ORDER BY keyword ASC");
        $keyword_num = mysql_num_rows($keyword_result);
        for($j = 0; $j < $keyword_num; $j++){
				  if($film_ids != ""){
  					$film_ids .= "|";
					}
          $film_ids .= mysql_result($keyword_result, $j, "film_id");
        }

				if($film_ids != ""){
          $film_result = DBQuery("SELECT films.id, users_films.id, films.title, films.duration, colors.color FROM wmc_films AS films, wmc_users_films AS users_films, wmc_colors AS colors WHERE films.id=users_films.film_id AND films.color_id=colors.id AND users_films.user_id=" . $user_id . " AND (films.title REGEXP '" . $search_filter . "+' OR films.duration REGEXP '" . $search_filter . "+' OR colors.color REGEXP '" . $search_filter . "+' OR films.id REGEXP '^(" . $film_ids . ")$') ORDER BY films.title ASC");
				} else{
          $film_result = DBQuery("SELECT films.id, users_films.id, films.title, films.duration, colors.color FROM wmc_films AS films, wmc_users_films AS users_films, wmc_colors AS colors WHERE films.id=users_films.film_id AND films.color_id=colors.id AND users_films.user_id=" . $user_id . " AND (films.title REGEXP '" . $search_filter . "+' OR films.duration REGEXP '" . $search_filter . "+' OR colors.color REGEXP '" . $search_filter . "+') ORDER BY films.title ASC");
				}
      } else if($char_filter != ""){
        if($char_filter == "other"){
          $film_result = DBQuery("SELECT films.id, users_films.id, films.title, films.duration, colors.color FROM wmc_films AS films, wmc_users_films AS users_films, wmc_colors AS colors WHERE films.title REGEXP '^[^a-z]' AND films.id=users_films.film_id AND films.color_id=colors.id AND users_films.user_id=" . $user_id . " ORDER BY films.title ASC");
        } else{
          $film_result = DBQuery("SELECT films.id, users_films.id, films.title, films.duration, colors.color FROM wmc_films AS films, wmc_users_films AS users_films, wmc_colors AS colors WHERE films.title REGEXP '^" . $char_filter . "' AND films.id=users_films.film_id AND films.color_id=colors.id AND users_films.user_id=" . $user_id . " ORDER BY films.title ASC");
        }
      } else{
      ...
      if($film_num > 0){
        ...
        for($i = 0; $i < $film_num; $i++){
          ...
          $tmpl->addVar("rows", "KEYWORD", "&amp;nbsp;");
          $tmpl->addVar("rows", "TRAILER", "&amp;nbsp;");
          $tmpl->addVar("rows", "HAS_CAST", "");
          $tmpl->addVar("rows", "HAS_KEYWORD", "");
          ...
          //Cast
          if($show_cast){
            $cast_result = DBQuery("SELECT casts.id, casts.name FROM wmc_cast AS casts, wmc_films_cast AS films_cast WHERE casts.id=films_cast.cast_id AND films_cast.film_id='" . $film_id . "' ORDER BY casts.name ASC");
            $cast_num = mysql_num_rows($cast_result);
            if($cast_num > 0){
            ...
            $val = "";
            for($j = 0; $j < $cast_num; $j++){
              $val .= "&lt;a onClick=\"postData('view_cast.php', 'VIEW_CAST', 'film_id=" . $film_id . "&amp;cast_id=" . mysql_result($cast_result, $j, "casts.id") . "');\"&gt;" . stripslashes(mysql_result($cast_result, $j, "casts.name")) . "&lt;br&gt;";
            }
          ...
          //Keywords
          if($show_keyword){
            $keyword_result = DBQuery("SELECT keyword FROM wmc_films_keywords WHERE film_id='" . $film_id . "' ORDER BY keyword ASC");
            $keyword_num = mysql_num_rows($keyword_result);
            if($keyword_num > 0){
              $tmpl->addVar("rows", "HAS_KEYWORD", "[Show Keywords]");
            }
            $val = "";
            for($j = 0; $j < $keyword_num; $j++){
              $val .= stripslashes(mysql_result($keyword_result, $j, "keyword")) . "&lt;br&gt;";
            }
            $tmpl->addVar("rows", "KEYWORD", $val);
          }
