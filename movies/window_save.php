<?php
  ...
  if (substr_count($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')){
	...
	// Set template
	$template_file = "window_save.tmpl.xml";
  ...
  if($_POST && mysql_num_rows($res) > 0){
    ...
    $x = htmlspecialchars(urlDecode($_POST['x']));
    $y = htmlspecialchars(urlDecode($_POST['y']));
    if($x < 0){
      $x = 0;
    }
    if($y < 0){
    ...
    if(mysql_num_rows($window_check) > 0){
      DBQuery("UPDATE wmc_users_windows SET x='" . $x . "', y='" . $y . "' WHERE user_id=" . $user_id . " AND window='" . $window . "'");
    } else{
      DBQuery("INSERT INTO wmc_users_windows (x, y, window, user_id) VALUES ('" . $x . "', '" . $y . "', '" . $window . "', '" . $user_id . "')");
    }
  }
