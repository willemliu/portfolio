...
* @version		2.4 ($Id: patTemplate.php,v 1.1 2008/06/13 08:14:25 wliu Exp $)
*/
class	patTemplate
{
...
*/
	function	createParser( $fname )
	{
		...
		while( !feof( $fp ) )
		{
			...
			$pos = ( count( $this->whitespace )-1 );
			if($pos > -1)
			{
  			switch( $this->whitespace[$pos] )
  			{
  				case	"trim":
  					$line	=	trim( $line );
  					break;
			...
			if	( preg_match( "/<patTemplate:([[:alnum:]]+)[[:space:]]*(.*)>/i", $line, $regs ) )			
			{
				...
				if( isset($attributes['keep']) && $attributes['keep'] > 0 )
				{
					...
					$newkeep	=	$attributes['keep'] > 1 ? " keep=\"".($attributes['keep'])."\"" : "";
...
*/
	function	startElementHandler( $fname, $tagname, $attributes, $line, $lineno )
	{
		...
		else if(( count( $this->whitespace )-1 ) > -1)
			...
			case "tmpl":
				...
				switch	( $this->template_types[$this->depth] )
				{
					...
					case "SIMPLECONDITION":
						//	none found => there is an error
						if( isset($attributes['requiredvars']) && $requiredvars = $attributes['requiredvars'] )
...
*/
	function	getVar( $template, $var )
	{
		...
		{
		  if(!isset($this->variables[$template][$var]))
		    $this->variables[$template][$var] = "";
