  ...
  if(mysql_num_rows($res) > 0){
    ...
    if(mysql_num_rows($menu_result) > 0 && $security_level >= mysql_result($menu_result, 0, "security_level")){
      ...
      if(mysql_num_rows($window_check) > 0){
        DBQuery("UPDATE wmc_users_windows SET opened='1', page='search_members_list.php', parameter='search_filter=" . urlencode($search_filter) . "' WHERE user_id=" . $user_id . " AND window='MEMBERS_LIST'");
      } else{
        DBQuery("INSERT INTO wmc_users_windows (x, y, opened, page, parameter, window, user_id) VALUES ('80', '110', '1', 'search_members_list.php', 'search_filter=" . urlencode($search_filter) . "', 'MEMBERS_LIST', '" . $user_id . "')");
      }

      //Show members
      if($search_filter != ""){
        $members_result = DBQuery("SELECT wmc_users.id AS id, wmc_users.nick_name AS nick_name, count(wmc_users_films.id) AS films_total FROM wmc_users, wmc_users_films WHERE wmc_users.nick_name REGEXP '" . $search_filter . "' AND wmc_users.id=wmc_users_films.user_id GROUP BY wmc_users.id");
      } else{
