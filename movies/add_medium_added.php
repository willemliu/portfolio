<?php
  ...
  if (substr_count($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')){
	...
	// Set template
  $template_file = "add_medium_added.tmpl.xml";
  ...
  $tmpl->addVar("setTimeout", "COMMAND", "document.getElementById('add_medium_add').disabled = true");
  ...
  if($_POST && mysql_num_rows($res) > 0){
    ...
    if(mysql_num_rows($section_result) > 0 && $security_level >= mysql_result($section_result, 0, "security_level")){
      ...
      if(mysql_num_rows($user_film_result) > 0){
        ...
        DBQuery("DELETE FROM wmc_films_mediums WHERE film_id='" . $film_id . "' AND user_id=" . $user_id);
        if(count($_POST['add_medium_id']) > 0){
          foreach($_POST['add_medium_id'] as $key => $val){
            $medium_id = mysql_real_escape_string(utf8_decode(htmlspecialchars(urlDecode($val))));
            DBQuery("INSERT INTO wmc_users_films_mediums (user_film_id, medium_id) VALUES ('" . $user_film_id . "', '" . $medium_id . "')");
            DBQuery("INSERT INTO wmc_films_mediums (film_id, medium_id, user_id) VALUES ('" . $film_id . "', '" . $medium_id . "', '" . $user_id . "')");
          }
        }
      }
      
      //Fade out
      FadeOut($tmpl, "ADD_MEDIUM", 0);
      $tmpl->addVar("setTimeout", "COMMAND", "postData('window_close.php', '', 'window=ADD_MEDIUM')");
      $tmpl->addVar("setTimeout", "MILLISECONDS", "0");
      $tmpl->parseTemplate("setTimeout", "a");
      
      $tmpl->addGlobalVar("INFORMATION", "Mediums added to your film.");
      ...
      $tmpl->addVar("setTimeout", "COMMAND", "document.getElementById('add_medium_add').disabled = false");
