<?php
  ...
  if (substr_count($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')){
	...
	// Set template
  $template_file = "add_genre_added.tmpl.xml";
  ...
  $tmpl->addVar("setTimeout", "COMMAND", "document.getElementById('add_genre_add').disabled = true");
  ...
  if($_POST && mysql_num_rows($res) > 0){
    ...
    if(mysql_num_rows($section_result) > 0 && $security_level >= mysql_result($section_result, 0, "security_level")){
      ...
      if(mysql_num_rows($user_film_result) > 0){
        ...
        DBQuery("DELETE FROM wmc_films_genres WHERE film_id='" . $film_id . "' AND user_id='" . $user_id . "'");
        if(count($_POST['add_genre_id']) > 0){
          foreach($_POST['add_genre_id'] as $key => $val){
            $genre_id = mysql_real_escape_string(utf8_decode(htmlspecialchars(urlDecode($val))));
            DBQuery("INSERT INTO wmc_films_genres (film_id, genre_id, user_id) VALUES ('" . $film_id . "', '" . $genre_id . "', '" . $user_id . "')");
          }
        }
      }
      
      //Fade out
      FadeOut($tmpl, "ADD_GENRE", 0);
      $tmpl->addVar("setTimeout", "COMMAND", "postData('window_close.php', '', 'window=ADD_GENRE')");
      $tmpl->addVar("setTimeout", "MILLISECONDS", "0");
      $tmpl->parseTemplate("setTimeout", "a");
      
      $tmpl->addGlobalVar("INFORMATION", "Genres added to your film.");
      ...
      $tmpl->addVar("setTimeout", "COMMAND", "document.getElementById('add_genre_add').disabled = false");
