<?php
  ...
  if (substr_count($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')){
	...
	// Set template
  $template_file = "remove_film.tmpl.xml";
  ...
  if($_POST && mysql_num_rows($res) > 0){
    ...
    $section_result = DBQuery("SELECT security_level FROM wmc_sections_security_levels WHERE section='Remove Film'");
    ...
    if(mysql_num_rows($section_result) > 0 && $security_level >= mysql_result($section_result, 0, "security_level")){
      ...
      $users_films_result = DBQuery("SELECT id FROM wmc_users_films WHERE user_id='" . $user_id . "' AND film_id='" . $film_id . "'");
      if(mysql_num_rows($users_films_result) > 0){
        $user_film_id = mysql_result($users_films_result, 0, "id");
        DBQuery("DELETE FROM wmc_users_films WHERE id='" . $user_film_id . "'");
      ...
      $tmpl->addGlobalVar("INFORMATION", "Film removed from your library.");
