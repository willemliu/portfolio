  ...
  if(mysql_num_rows($res) > 0){
	...
	if(mysql_num_rows($section_result) > 0 && $security_level >= mysql_result($section_result, 0, "security_level")){
      ...
      if(mysql_num_rows($window_check) > 0){
        DBQuery("UPDATE wmc_users_windows SET opened='1', page='search_member_films.php', parameter='search_filter=" . urlencode($search_filter) . "&char=" . $char_filter . "' WHERE user_id=" . $user_id . " AND window='MEMBER_FILMS'");
      } else{
        DBQuery("INSERT INTO wmc_users_windows (x, y, opened, page, parameter, window, user_id) VALUES ('80', '110', '1', 'search_member_films.php', 'search_filter=" . urlencode($search_filter) . "&char=" . $char_filter . "', 'MEMBER_FILMS', '" . $user_id . "')");
      }

      //Check which columns to show or hide
      $column_result = DBQuery("SELECT * FROM wmc_users_films_columns WHERE user_id=" . $member_id);
      ...
      $tmpl->addVar("setTimeout", "COMMAND", "document.getElementById('member_films_search_filter').focus();");
