<?php
  ...
  if (substr_count($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')){
	...
	// Set template
  $template_file = "remove_trailer.tmpl.xml";
  ...
  if($_POST && mysql_num_rows($res) > 0){
    ...
    $section_result = DBQuery("SELECT security_level FROM wmc_sections_security_levels WHERE section='Remove Trailer'");
    ...
    $trailer_id = mysql_real_escape_string(htmlspecialchars(urlDecode($_POST['trailer_id'])));
    $users_films_result = DBQuery("SELECT id, user_id FROM wmc_films_trailers WHERE id='" . $trailer_id . "'");
    if(mysql_num_rows($users_films_result) > 0){
      if(mysql_result($users_films_result, 0, "user_id") == $user_id || $security_check){
        //Delete
        DBQuery("DELETE FROM wmc_films_trailers WHERE id='" . $trailer_id . "'");
        DBQuery("DELETE FROM wmc_users_films_trailers_reports WHERE trailer_id='" . $trailer_id . "'");

        //Fade Information box in immediatly
        FadeIn($tmpl, "INFORMATION", 0);
        $tmpl->addGlobalVar("INFORMATION", "Trailer removed from this film.");

        //Fade View Trailer
        FadeOut($tmpl, "VIEW_TRAILER", 0);
        $tmpl->addVar("setTimeout", "COMMAND", "postData('window_close.php', '', 'window=VIEW_TRAILER')");
        ...
        FadeIn($tmpl, "INFORMATION", 0);
        $tmpl->addGlobalVar("INFORMATION", "You can only delete trailers you've submitted.");
      }
    } else{
      FadeIn($tmpl, "INFORMATION", 0);
      $tmpl->addGlobalVar("INFORMATION", "Trailer not found in the database.");
    }
  } else{
    //Fade Information box in immediatly
    FadeIn($tmpl, "INFORMATION", 0);
